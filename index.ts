...
const sessionMiddleware = async (
  ...
  try {
    ...
    res.locals.user = await findUserById(session.userId as unknown as string); // Attach user to request
...
app.post("/signin", async (req: Request, res: AppRes) => {
...
app.post("/admin_one", async (req: Request, res: AppRes) => {
  try {
